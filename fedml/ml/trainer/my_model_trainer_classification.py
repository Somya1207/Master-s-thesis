...
class ModelTrainerCLS(ClientTrainer):
    ...
    def train_gmm(self, client_data, device, args,g_global,f_global,w_locals_prev,additional_epochs):
        ans = 0
        g = self.g
        f = self.f
    ...
    # loop through training data
        for epoch in range(args.epochs+additional_epochs):
            for batch in client_data:
                x_batch = batch[2]
                y_batch = batch[3]
                z_batch = batch[4]
                g_obj, f_obj = self.game_objective.calc_objective(
                    g, f, x_batch, z_batch, y_batch,g_global,f_global,w_locals_prev)
                # do single step optimization on f and g
                if epoch == 0:
                    ans = g_obj.item() + f_obj.item()
